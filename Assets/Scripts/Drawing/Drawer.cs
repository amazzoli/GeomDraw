    ...
    public class Drawer
    {
            ...
            Color color)
        {
            if (renderer.GetComponent<DrawnSprite>() == null)
                renderer.gameObject.AddComponent<DrawnSprite>();
            renderer.GetComponent<DrawnSprite>().Init();
            ...
            renderer.sprite = Sprite.Create(tex, rect, new Vector2(0.5f, 0.5f), pixelsPerUnity);
        ...
        public void Draw(SpriteRenderer renderer, IDrawable drawable, bool updateDrawnSprite = true)
        {
            ...
            if (!drawable.CheckDrawability(renderer.sprite.pixelsPerUnit))
            {
            ...
            if (updateDrawnSprite)
            {
                if (renderer.GetComponent<DrawnSprite>() == null)
                    renderer.gameObject.AddComponent<DrawnSprite>();
                renderer.GetComponent<DrawnSprite>().NewDraw(drawable);
            }
        ...
        private void DrawShape(SpriteRenderer renderer, IDrawableShape shape)
        {
            ...
            int ni = canvas.height, nj = canvas.width;

            Color[] pixels = canvas.GetPixels();
            (float[] aa, int[] rect) = AntialiaseShape(PixelCoords(shape.Border(pixelsPerUnit), pixelsPerUnit));
        ...
        private void DrawLine(SpriteRenderer renderer, IDrawableLine line)
        {
            ...
            Texture2D canvas = renderer.sprite.texture;
            int ni = canvas.height, nj = canvas.width;
            (Vector2[] discrLeft, Vector2[] discrRight) = line.LeftRightDiscretization(pixelsPerUnit);
            discrLeft = PixelCoords(discrLeft, pixelsPerUnit);
            discrRight = PixelCoords(discrRight, pixelsPerUnit);
